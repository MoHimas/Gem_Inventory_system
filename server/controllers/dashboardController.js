...
const getDashboardStats = async (req, res) => {
  try {
    ...
    const totalSalesRes = await pool.query(
      "SELECT SUM(total_price) as total FROM sales WHERE user_id = $1",
      [req.user.id],
    );
    const totalSales = totalSalesRes.rows[0].total || 0;

    //Total Stock Value
    const stockValueRes = await pool.query(
      "SELECT SUM(total_price) as total FROM gemstones WHERE user_id = $1",
      [req.user.id],
    );
    ...
    const suppliersRes = await pool.query(
      "SELECT COUNT(*) as count FROM suppliers WHERE user_id = $1",
      [req.user.id],
    );
    ...
    const lowStockRes = await pool.query(
      "SELECT COUNT(*) as count FROM gemstones WHERE quantity < 5 AND user_id = $1",
      [req.user.id],
    );
    const lowStockCount = lowStockRes.rows[0].count;

    // Total Receivable (Pending payments from Sales)
    ...
    // Total Payable (Pending payments to Suppliers)
    const payableRes = await pool.query(
      "SELECT SUM(total_price - paid_amount) as total FROM purchases WHERE user_id = $1",
      [req.user.id],
    );
    ...
    //Stock Distribution by Type (Count and Value)
    ...
    // Sales Trend (Last 4 months: Current + 3 previous)
    ...
    // Payment Status Distribution (Cash Flow Health)
