...
exports.getSpecificInsight = async (req, res) => {
  try {
    ...
    let specificGoal = "";
    let contextDataObj = {};
    const { rows: testData } = await db.query(
      `SELECT 1 FROM gemstones WHERE user_id = $1 LIMIT 1`,
      [userId],
    );
    ...
    if (testData.length === 0) {
      return res.status(404).json({
        error: "NO_DATA",
        details: "No active inventory found for analysis.",
      });
    }
    ...
    switch (type) {
      case "velocity":
        ...
        specificGoal =
          "Analyze the fastest moving attribute combinations (shape, type, color) over the last 60 days. Provide a clear decision stating what exact attributes to allocate budget towards to keep cash moving fast.";
      ...
      case "dead_stock":
        analysisType = "Capital Efficiency (Cash Flow Decisions)";
        specificGoal =
          "Identify high-value gems in stock for over 45 days. Provide a specific liquidation decision (e.g., private viewing, trade discount) to free up the locked capital and reinvest in faster-moving stones.";
      ...
      case "matched_pair":
        ...
        specificGoal =
          "Find two stones with similar shape, type, color, clarity, and weight. Suggest bundling them as a 'Matched Pair' for earrings with a recommended % price increase over individual values.";
      ...
      case "recutting":
        analysisType = "Artisan Value-Add (Gemstone Improvements)";
        specificGoal =
          "Identify stones that have high color/clarity but have been unsold for over 30 days. Suggest sending them to a lapidary for a precision recut to improve brilliance, noting that weight loss is offset by price-per-carat increase.";
      ...
      case "clarity_roi":
        ...
        specificGoal =
          "Analyze profit margins based on clarity grades. Compare the ROI of Flawless vs lower grades (like VS1) and make a sourcing decision to maximize Return on Investment.";
      ...
      case "hue_shift":
        analysisType = "Market Trend Alignment (Procurement Strategy)";
        specificGoal =
          "Evaluate the current color distribution of unsold stock against recently sold color trends. Suggest which exact color families to acquire to meet current demand.";
        ...
        contextDataObj = {
          recent_trending_sold_colors: soldHues,
          currently_overstocked_colors: stockHues,
        };
        break;
      ...
      case "concentration":
        ...
        specificGoal =
          "Calculate financial exposure to the single most dominant gemstone type in inventory. Recommend specific semi-precious or alternative varieties to diversify the next purchases.";
      ...
      case "supplier_reliability":
        analysisType = "Risk & Resilience (Stability Decisions)";
        specificGoal =
          "Compare suppliers by the amount of dead stock they yield. Suggest pivoting sourcing volume away from the riskiest supplier towards the safest one.";
      ...
      case "receivables_risk":
        ...
        specificGoal =
          "Analyze outstanding customer balances. Identify customers who owe the most and how long the debt has been aging. Recommend a 'Payment Incentive' or 'Collection Priority' based on the risk level.";
      ...
      case "payables_strategy":
        analysisType = "Financial Liquidity (Payments & Cash Flow)";
        specificGoal =
          "Analyze upcoming supplier obligations. Compare outstanding debt vs. the performance of the stock bought from that supplier. Suggest which supplier to pay first to maintain crucial sourcing relationships.";
      ...
      default:
        return res
          .status(400)
          .json({ error: "Invalid insight type requested." });
    }
    ...
    try {
      ...
      parsedInsight = {
        ...
        decision:
          parts[1]?.trim() ||
          "Consult your inventory logs for immediate action.",
      };
    }

    // Final Sanitization: Ensure no stray "$" symbols remain in the output
    const sanitize = (text) =>
      typeof text === "string" ? text.replace(/\$/g, "LKR ") : text;
    ...
    if (err.status === 429) {
      return res.status(429).json({
        ...
        message:
          "You have exceeded the daily AI request limit (Free Tier). Please wait a few hours or upgrade your Google AI plan.",
    ...
    res
      .status(500)
      .json({ error: "Intelligence engine failure. Please check logs." });
  }
};
